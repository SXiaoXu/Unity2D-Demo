...
public class LeaderboardListView : MonoBehaviour
{
    ...
    //           // 设置 ListView 的数据源
    //         var myDataList = new List<MyData>();  // MyData 是你自定义的数据类型
    //         listView.itemsSource = myDataList;

    //         // 创建列表项模板
    //         // listView.itemsSource = new VisualElement(CreateItem);

    //         // 将 ListView 添加到当前 VisualElement 中
    //     }
    //  // 列表项模板的创建函数
    //     private VisualElement CreateItem()
    //     {
    //         // 创建 VisualElement 作为列表项容器
    //         var item = new VisualElement();

    //         // 创建并添加需要绑定数据的 UI 元素
    //         var nameLabel = new Label();
    //         nameLabel.bindingPath = "name";  // 将 Label 的文本与数据源中的 name 属性绑定
    //         item.Add(nameLabel);

    //         var ageLabel = new Label();
    //         ageLabel.bindingPath = "age";  // 将 Label 的文本与数据源中的 age 属性绑定
    //         item.Add(ageLabel);
    ...
    protected virtual void Awake()
    {
        ...
        contentRT.anchorMin = new Vector2(contentRT.anchorMin.x,0 );
        contentRT.anchorMax = new Vector2(contentRT.anchorMax.x,0 );
    ...
    protected virtual void AppearCells()
    {
        foreach (int index in appearIndexes)
        {
            ...
            //设置Cell数据，对Cell进行初始化
            cellRT.GetComponent<ListViewCell>().Refresh(index);
        }
    }
